...
export function planSpatialScene({ intent, assets = [], device }) {
  ...
  const sceneSpec = scene({
    id: `plan-${intent.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'default'}`,
    ...
    nodes: assets.map((asset, index) => ({
      ...
      transform: transform({ x: index * 0.5, z: index * -0.25, scale: asset.scale ?? 1 }),
      data: { assetId: asset.id ?? `asset-${index + 1}`, requires: asset.requires ?? [] },
    })),
  });
