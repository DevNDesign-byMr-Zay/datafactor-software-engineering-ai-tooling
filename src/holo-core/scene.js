...
export function transform(input = {}) {
  ...
  return Object.freeze({
    ...
    rx: finite(input.rx ?? 0, 'rx'), ry: finite(input.ry ?? 0, 'ry'), rz: finite(input.rz ?? 0, 'rz'), scale,
  });
}
...
export function scene({ id, nodes = [], metadata = {} } = {}) {
  ...
  const normalizedNodes = nodes.map((node, index) => {
    ...
    return Object.freeze({ id: nodeId, kind: node.kind ?? 'content', transform: transform(node.transform), visible: node.visible !== false, data: Object.freeze({ ...(node.data ?? {}) }) });
  });
  return Object.freeze({ schema: 'holo.scene.v1', id: id.trim(), metadata: Object.freeze({ ...metadata }), nodes: Object.freeze(normalizedNodes) });
}
...
export function negotiate(sceneSpec, deviceSpec) {
  ...
  const missing = [...new Set(required.map(String))].filter((capability) => !deviceSpec.capabilities.includes(capability));
