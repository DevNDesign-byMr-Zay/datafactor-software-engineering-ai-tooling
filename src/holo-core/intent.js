...
export function interpretHolographicIntent({ prompt, sceneType = 'presentation', displayType = 'projector', assetIds = [], constraints = {}, animation = {} } = {}) {
  ...
  if (!DISPLAY_TYPES.includes(displayType)) throw new TypeError(`Unsupported display type: ${displayType}`);
  if (!constraints || typeof constraints !== 'object' || Array.isArray(constraints)) throw new TypeError('constraints must be an object.');
  if (!animation || typeof animation !== 'object' || Array.isArray(animation)) throw new TypeError('animation must be an object.');
