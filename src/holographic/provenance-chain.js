...
export function validateHolographicProvenanceBinding({ envelope, snapshotId, sceneId, provenanceRef, scene = null, sceneFingerprint = null } = {}) {
  if (!validateHolographicEvidenceEnvelope(envelope)) return false;
  const identityMatches = envelope.snapshotId === text(snapshotId, 'snapshotId')
  ...
  if (scene != null || sceneFingerprint != null) {
    if (scene == null || typeof sceneFingerprint !== 'string' || !verifyHolographicSceneFingerprint(scene, sceneFingerprint)) return false;
    if (scene.sceneId !== text(sceneId, 'sceneId') || scene.snapshotId !== text(snapshotId, 'snapshotId')) return false;
  }
  return true;
}

export function createHolographicProvenanceBinding({ envelope, snapshotId, sceneId, provenanceRef, scene = null, sceneFingerprint = null } = {}) {
  if (!validateHolographicProvenanceBinding({ envelope, snapshotId, sceneId, provenanceRef, scene, sceneFingerprint })) {
