...
The current JavaScript suite contains **68 passing tests across 12 suites**. The measured JavaScript surface is **99.15% statements, 89.42% branches, 98.07% functions, and 100% lines**. CI fails below 85% statements/functions/lines or 75% branches. The maintained Python surface retains its ≥85% coverage gate.
