...
function safeRelative(value, label) {
  ...
  assert(
    ...
    `${label} must be an exact repository-relative path`,
  );
...
assert(
  Array.isArray(surface.maintainedRoots) && surface.maintainedRoots.length > 0,
  'maintainedRoots must be non-empty',
);
assert(
  Array.isArray(surface.promotedMaintainedArtifacts),
  'promotedMaintainedArtifacts must be an array',
);
...
assert(
  archive?.releaseCommit === '25bf8b9e36b327b3001f8b12cd0709cf7f1b84ad',
  'archive release commit drifted',
);
assert(
  archive?.releaseTree === 'cf7e5cfa5398cd8739b9bba2c6db5047f88e2231',
  'archive release tree drifted',
);
...
assert(
  archive?.canonicalInventorySha256 ===
    'cc5592a67d0d68009489b0cc1a6a575f553ff800e7b3d9775c62206d8f863409',
  'archive inventory digest drifted',
);

const archiveManifest = JSON.parse(
  await readFile(safeRelative(archive.manifest, 'historicalArchive.manifest'), 'utf8'),
);
...
assert(
  archiveManifest.release_tree === archive.releaseTree,
  'archive manifest release tree mismatch',
);
assert(
...
assert(
  archiveManifest.corpus_directory_count === archive.directoryCount,
  'archive manifest directory count mismatch',
);
assert(
  archiveManifest.corpus_total_bytes === archive.totalBytes,
  'archive manifest byte count mismatch',
);
assert(
...
assert(
  Array.isArray(archiveManifest.files) && archiveManifest.files.length === 1610,
  'archive manifest must list every released historical file',
);

const corpusRoot = safeRelative(
  archive.corpusRootAtRelease,
  'historicalArchive.corpusRootAtRelease',
);
assert(
  !(await exists(corpusRoot)),
  'historical corpus must remain outside the scored maintained tree',
);
...
for (const [index, artifact] of surface.promotedMaintainedArtifacts.entries()) {
  ...
  assert(
    ...
    `promotedMaintainedArtifacts[${index}] must reference a file`,
  );
