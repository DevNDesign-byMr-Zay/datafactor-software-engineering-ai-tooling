...
describe('holo-core spatial acceptance', () => {
  test('links normalized intent, scene, device and capability negotiation', () => {
    const intent = interpretHolographicIntent({ prompt: 'Show the product in depth', sceneType: 'product', displayType: 'projector' });
    const target = device({ id: 'projector-1', type: 'projector', capabilities: ['depth'] });
    const plan = planSpatialScene({ intent, assets: [{ id: 'product', requires: ['depth'] }], device: target });
  ...
  test('produces stable fingerprints for identical acceptance inputs', () => {
    const intent = interpretHolographicIntent({ prompt: 'diagram', sceneType: 'diagram', displayType: 'holomat' });
    const target = device({ id: 'mat-1', type: 'holomat', capabilities: [] });
    const plan = planSpatialScene({ intent, assets: [{ id: 'diagram' }], device: target });
    const a = createSpatialAcceptanceReceipt({ intent: plan.intent, sceneSpec: plan.scene, device: target });
