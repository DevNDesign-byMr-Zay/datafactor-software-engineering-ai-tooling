...
describe('holographic provenance binding', () => {
  ...
  const scene = { sceneId: 'scene-001', snapshotId: 'snapshot-001', layers: { topology: true }, nodes: [{ id: 'node-1', position: { x: 1, y: 2, z: 3 } }] };
  ...
  test('binds a valid evidence envelope to its source identities', () => {
    ...
    const binding = createHolographicProvenanceBinding({ ...input, envelope, scene, sceneFingerprint: fingerprintHolographicScene(scene) });
  ...
  test('rejects a tampered scene fingerprint or cross-identity scene', () => {
    ...
    expect(validateHolographicProvenanceBinding({ envelope, ...input, scene, sceneFingerprint: `${fingerprint.slice(0, -1)}0` })).toBe(false);
    expect(validateHolographicProvenanceBinding({ envelope, ...input, scene: { ...scene, sceneId: 'scene-other' }, sceneFingerprint: fingerprint })).toBe(false);
  });
});
