...
describe('validated holographic scene handoff', () => {
  ...
  it('emits a verifiable immutable advisory handoff', () => {
    ...
    expect(handoff.acceptance).toMatchObject({ accepted: true, provenanceValid: true, fingerprintValid: true, safetyValid: true, authoritative: false, physicalActuation: false, advisoryOnly: true });
    expect(handoff.provenanceBinding).toMatchObject({ snapshotId: 'snap-handoff-1', sceneId: handoff.scene.sceneId, provenanceRef: 'prov-handoff-1' });
  });

  it('rejects scene mutation after handoff construction', () => {
    const handoff = build();
    const tampered = { ...handoff, scene: { ...handoff.scene, nodes: [{ ...handoff.scene.nodes[0], position: { x: 99, y: 2, z: 3 } }] } };
  ...
  it('rejects forged fingerprints and unsafe authority flags', () => {
    ...
    expect(verifyValidatedHolographicSceneHandoff({ ...handoff, safety: { authoritative: true, physicalActuation: false, advisoryOnly: true } })).toBe(false);
  ...
  it('rejects provenance binding drift even when the outer fingerprint is recomputed', () => {
    const handoff = build();
    const forged = { ...handoff, provenanceBinding: { ...handoff.provenanceBinding, provenanceRef: 'prov-handoff-attacker' } };
  ...
  it('re-validates the source envelope instead of trusting only the embedded fingerprint', () => {
    const planned = planHolographicScene({ snapshotId: 'snap-handoff-2', provenanceRef: 'prov-handoff-2', intent: 'inspect', target: 'holo-mat', objects: [{ id: 'node-2', kind: 'load', x: 4, y: 5, z: 6 }] });
    const handoff = createValidatedHolographicSceneHandoff({ envelope: planned.evidence, scene: planned.scene, snapshotId: 'snap-handoff-2', sceneId: planned.scene.sceneId, provenanceRef: 'prov-handoff-2' });
  ...
  it('rejects an envelope swap even when the attacker keeps the handoff binding unchanged', () => {
    const planned = planHolographicScene({ snapshotId: 'snap-handoff-3', provenanceRef: 'prov-handoff-3', intent: 'inspect', target: 'holo-mat', objects: [{ id: 'node-3', kind: 'load', x: 7, y: 8, z: 9 }] });
